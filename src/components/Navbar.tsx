  ...
  return (
    <header className="sticky top-0 z-40 transition-colors">
      ...
      <div
        ...
        style={{ background: 'var(--glass-bg)', backdropFilter: 'var(--blur-glass)', WebkitBackdropFilter: 'var(--blur-glass)' }}
      >
            ...
            <nav className="hidden md:flex items-center gap-0.5" aria-label="Main Navigation">
              {categories.map((cat) => {
                ...
                return (
                  <button
                    key={cat.id}
                    onClick={() => { onSelectCategory(cat.id); onSearchChange(''); }}
            ...
            >
              ...
              <input
                ...
                style={{
                  ...
                  borderColor: searchFocused ? 'var(--color-accent)' : 'rgba(255,255,255,0.08)',
                  boxShadow: searchFocused ? '0 0 0 1px var(--color-accent)' : 'none',
                }}
              />
              {searchQuery && (
                <button
      ...
      {mobileMenuOpen && (
        <div
          ...
          style={{ background: 'var(--glass-bg)', backdropFilter: 'var(--blur-glass)' }}
        >
          {categories.map((cat) => {
            const active = currentCategory === cat.id && !searchQuery;
            return (
              <button
                key={cat.id}
                onClick={() => { onSelectCategory(cat.id); onSearchChange(''); setMobileMenuOpen(false); }}
                ...
                style={active ? { background: 'rgba(225,29,72,0.15)', borderLeft: '2px solid var(--color-accent)' } : {}}
              >
                {cat.icon}
                {cat.label}
              </button>
