...
export const ContinueWatchingRow: React.FC<ContinueWatchingRowProps> = ({
  ...
  return (
    <section className="mb-10 animate-slideUp">
      ...
      <div className="flex items-stretch gap-4 overflow-x-auto pb-4 pt-1 px-1 scrollbar-none snap-x snap-mandatory">
        {history.map((entry) => {
          const isTv =
            entry.season !== undefined && entry.episode !== undefined;
          const subLabel = isTv
            ? `S${entry.season} · E${entry.episode}`
            : entry.item.genres[0] || "Movie";
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(to top, rgba(0,0,0,0.90) 0%, rgba(0,0,0,0.30) 50%, transparent 100%)",
                  }}
                />
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                  <button
                    ...
                    className="w-12 h-12 rounded-full flex items-center justify-center text-white transition-all cursor-pointer transform scale-90 group-hover:scale-100 group-focus-within:scale-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950"
                    style={{
                      background: "var(--color-accent)",
                      boxShadow: "0 0 20px var(--color-accent-glow)",
                    }}
                    title="Resume"
                    aria-label={`Resume watching ${entry.item.title}`}
                  >
                    <Play
                ...
                <button
                  ...
                  className="absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center border opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400 transition-all cursor-pointer text-zinc-400 hover:text-rose-300"
                  style={{
